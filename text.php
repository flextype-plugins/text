...
Event::addListener('onShortcodesInitialized', function () {
    ...
    Entries::shortcode()->addHandler('text-cut', function(ShortcodeInterface $s) {
    ...
    Entries::shortcode()->addHandler('text-lowercase', function(ShortcodeInterface $s) {
        return Text::lowercase($s->getContent());
    });

    // Shortcode: [text-uppercase]Text here[/text-uppercase]
    Entries::shortcode()->addHandler('text-uppercase', function(ShortcodeInterface $s) {
        return Text::uppercase($s->getContent());
    });

    // Shortcode: [text-lorem num=3]
    Entries::shortcode()->addHandler('text-lorem', function(ShortcodeInterface $s) {
        return Text::lorem(((null !== $s->getParameter('num')) ? $s->getParameter('num') : 1 ));
    });

    // Shortcode: [text-right num=3]Text here[/text-right]
    Entries::shortcode()->addHandler('text-right', function(ShortcodeInterface $s) {
        return Text::right($s->getContent(),
                           $s->getParameter('num'));
    });

    // Shortcode: [text-left num=3]Text here[/text-left]
    Entries::shortcode()->addHandler('text-left', function(ShortcodeInterface $s) {
